...
class Intervals:
    ...
    def create_intervals_ball(self, base):
        intervals_random = random.choice([1, 3, 5])
        if intervals_random == 1:
            intervals_ball = self.note_data[self.note_data.index(base) + 12]
            return intervals_ball, intervals_random
        elif intervals_random == 3:
            intervals_ball = self.note_data[self.note_data.index(base) + 4]
            return intervals_ball, intervals_random
        else:
            intervals_ball = self.note_data[self.note_data.index(base) + 8]
            return intervals_ball, intervals_random
    ...
    def create_balls(self):
        ...
        base_ball = random.choice(self.available_note)
        intervals_ball, intervals_random = self.create_intervals_ball(base_ball)
        all_ball = [base_ball, intervals_ball]
        for i in range(2):
            ...
            if self.level <= 3:
                ball = harmonic_ball.Ball(radius, x, y, vx, vy, all_ball[i]["color"], all_ball[i]["note"],
                                          all_ball[i]["label"])
            else:
                ball = harmonic_ball.Ball(radius, x, y, vx, vy, (100,100,100), all_ball[i]["note"],
                                          all_ball[i]["label"])
            self.ball_list.append(ball)
        self.answer = intervals_random
    def create_buttons(self):
        button_width = 125
        button_height = 50
        gap = 40
        start_x = -self.canvas_width + 285 - gap
        start_y = -self.canvas_height - gap - 13
        pad = [{"label": "Major 3rd", "Ans": 3}, {"label": "Perfect 5th", "Ans": 5}, {"label": "Octave", "Ans": 1}]
        for i, note in enumerate(pad):
            x = start_x + i * (button_width + gap)
            button = Button(x, start_y, button_width, button_height, note["label"], note["Ans"],
